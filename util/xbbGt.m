function varargout = xbbGt( action, varargin )
...
if( format==0 )
  ...
  bb=[in{2} in{3} in{4} in{5}]; 
  bbv=[in{7} in{8} in{9} in{10}];
...
% only keep objects whose lbl is in lbls or ilbls %仅保留lbl与lbls或ilbls中标签相同的
...
if(~isempty(vRng)),  for i=1:n, o=objs(i); bb=o.bb; bbv=o.bbv; %#ok<ALIGN>
    if(~o.occ || all(bbv==bb)), v=1; 
    elseif(all(bbv==0)), v=0;
    else v=(bbv(3)*bbv(4))/(bb(3)*bb(4)); 
    end
...
for i=1:m
  for j=1:n
    ...
    t=w*h; 
    if(ig(j)) %u是分母，不考虑gt的面积
        u=da(i); 
    else %dt和gt面积相加，减去相交区域
        u=da(i)+ga(j)-t; 
    end; 
    oa(i,j)=t/u;
  end
