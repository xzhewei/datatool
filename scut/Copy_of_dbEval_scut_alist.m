...
for g=1:nGt
  for d=1:nDt
    ...
    if(plotRoc==1), ys{g,d}=1-ys{g,d}; score=1-score; end
    if(plotRoc==1), score=exp(mean(log(score))); else score=mean(score); end
    scores(g,d)=score;
  end
end
...
stra={res(1,:).stra}; stre={res(:,1).stre}; scores1=scores; % scores1=round(scores*100);
...
for p=1:nPlots
  ...
  if( plotAlg )
    ...
    for d=1:nDt, lgd1{d}=sprintf('%d %s',scores1(p,d),stra{d}); end
