...
fps       = 10;  % initial fps for playback (if 0 uses actual fps)
skip0     = 1;   % initial skip (zoom)
seqPad    = 4;    % amount of padding around object in seq view
siz0      = 10;   % minimum rect width/height
...
if(~isempty(annNm)), filesApi.openAnn(annNm); end
  ...
  function makeLayout()
    ...
    pObj.hBtSpt = uic(pObj.h,btnPr{:},[350 3 25 25],ha,'Center'); %添加切分按钮
    ...
    function keyPress( h, evnt ) %#ok<INUSL>
      ...
      if( char==119 ), dispApi.setFrame('+'); end
      if( char==113 ), dispApi.setFrame('-'); end
      if( char==97 ), dispApi.setOffset('-'); end
      if( char==115 ), dispApi.setOffset('+'); end
    end
  ...
  function api = objMakeApi()
    ...
    set(pObj.hBtSpt, 'callback', @(h,e) objSplit()); %建立对应函数
    ...
    api=struct( 'init',@init, 'drawRects',@drawRects, ...
      ...
      'annForSave',@annForSave, 'annSaved',@annSaved, 'objMoveDown', @objMoveDown, 'objLockCurrent', @objLockCurrent );
...
%               static=(lockSet && id~=objId) || 1; %一直显示标签
%             hs1=drawRect(o.pos,o.posv,lims,lbl,static,id,ind,-1); %原函数
            hs1=drawRect(o.pos,o.posv,lims,lbl,static,id,ind,-1,o.occl); %增加o.occl参数
            hs=[hs hs1]; %#ok<AGROW>
          end
        case 'objSeq'
...
%             hs1=drawRect(o.pos,o.posv,o.lims,lbl,0,id,ind,i-1); %原函数
            hs1=drawRectSeq(o.pos,o.posv,o.lims,lbl,0,id,ind,i-1); %使用单独的函数进行绘制
            hs=[hs hs1]; %#ok<AGROW>
          end
      end
      
      %新增的专门用于显示下方16个窗口标注框的函数
      function hs = drawRectSeq(pos,posv,lims,lbl,static,id,ind,sid)
      ...
      function hs = drawRect(pos,posv,lims,lbl,static,id,ind,sid,isocc)
        if(colType), t=find(strcmp(lbl,objTypes)); else t=id; end
        col=rectCols(mod(t-1,size(rectCols,1))+1,:);
        if(id~=objId), ls='-'; else
          if(ind==objInd), ls='--'; else ls=':'; end; end
        ...
        if( static )
          api.setPosLock(1);
          ht=text(pos(1),pos(2)-10, [socc lbl]); hs=[hs ht]; %把'OC-'添加到标签上
          set(ht,'color','w','FontSize',10,'FontWeight','bold');
        else
          api.setPosSetCb( @(pos) objSetPos(pos(1:4),id,ind,sid) );
          %增加下面两行，使得在非播放的时候也显示标签
    ...
    function objMoveDown()
        t = vbb('get',A,objId);
        
    end
    
    function objLockCurrent()
        objSetLck(objId, objId);
    end
    ...
    function objSplit()
        assert(objId>0);
        o1 = vbb( 'get', A, objId, objS+1, curInd);
        o2 = vbb( 'get', A, objId, curInd+1);
        A = vbb( 'del', A, objId);
        objId = -1;
        dispApi.requestUpdate();
        o1.id = -1;
  ...
  function api = dispMakeApi()
    ...
    api = struct( 'requestUpdate',@requestUpdate, 'init',@init, ...
      ...
      'width',@width, 'height',@height, 'setPlay', @setPlay, 'setFrame', @setFrame );
    ...
    function setOffset( offset1 )
      if( nargin==1 )
        if(strcmp(offset1,'-'))
            if(offset > 1), offset = offset - 1; end
        elseif(strcmp(offset1,'+'))
            if(offset < 15), offset = offset + 1; end
        else
            offset=offset1; 
        end
        
      else
        offset=round(get(pRt.slOffst,'Value')); 
      end
